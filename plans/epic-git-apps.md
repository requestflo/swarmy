...
> pure planner, golden tests). Phases 2–6 below are the build order.
...
7. **Reading the commit is an agent command** (`git.inspect` on a Builder node):
   one path for every git host, the controller never clones. Provider APIs are
   used only to write feedback (checks, statuses, comments) and to list repos.
...
| 2    | Connections & auth: GitHub App manifest flow, installations, GitLab OAuth/token, generic PAT/deploy key; repo/branch picker; app-level webhook + dedupe; polling worker; `git.inspect` agent command | L    | `schema/cicd.prisma`; `services/git-connections.service.ts`, `services/git-providers/{github,gitlab,generic}.ts` (pure clients over injected `fetch`); `routers/gitConnections.ts`; `apps/api/src/{webhooks,git-callback}.ts`; `protocol/build.ts` + `apps/agent/src/handlers/git-inspect.ts`; `workers/app-poll.ts` |

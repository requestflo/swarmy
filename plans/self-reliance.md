...
| B5 | **Built.** Managed Postgres on `pgvector/pgvector:pg17` (official `postgres:17` + pgvector, rebuilt by the pgvector project on every upstream release) under swarmy's own boot layer `packages/core/src/manageddb-pg.ts` (the service `command`: replication role + pg_hba on first boot, `pg_basebackup` replicas via a side dir, failover rejoin that moves a demoted writer's PGDATA aside, never deletes it). `POSTGRES_*` + `SWARMY_PG_*` env; promotion is `pg_promote()`. Caches: official `valkey/valkey:8` / `redis:7.4`, sentinels = the engine's own `*-sentinel`. etcd DCS: `gcr.io/etcd-development/etcd`. HA templates ported (postgres-ha loses repmgr auto-failover: manual promotion, managed clusters do safe failover). Not yet: the new data images in the B3 mirror BOM. |

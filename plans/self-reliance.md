...
| **Tailscale SaaS** control plane (`controlplane.tailscale.com`) | `packages/mesh/src/drivers/tailscale.ts:12`; `packages/trpc/src/services/mesh.service.ts:620` | No (explicit driver) | Mesh control fails offline. | Keep as an opt-in driver. B1's in-cluster NetBird replaces it as the default (`epic-self-hosted-mesh-and-fleets.md`). |

...
|---|---|---|---|---|---|
| B1 | **NetBird Cloud** (`api.netbird.io`) as the mesh control plane | `scripts/install-swarmy.sh:417-429,484-515`; default driver `packages/trpc/src/services/mesh.service.ts:221`; `packages/mesh/src/drivers/netbird.ts` | This is the default the installer offers. Remote nodes' private network then depends on a SaaS: no enrolment or ACL change works without it, and the cluster's topology is held by a third party. The Headscale driver exists, but swarmy doesn't run Headscale itself. The WireGuard driver has no NAT traversal. | Swarmy deploys **Headscale as a system service** on the controller (pinned image, sqlite on a volume, ACL file already rendered by `buildHeadscaleAcl`). The installer adds `--mesh swarmy` and makes it the default. Nodes use the existing `tailscale/tailscale` client with `--login-server` pointed at the controller. DERP runs on the controller/edge (Headscale's embedded DERP), so no Tailscale DERP servers are needed. Keep NetBird (self-hosted or Cloud) and plain WireGuard as opt-in drivers. | L |
...
| **Tailscale SaaS** control plane (`controlplane.tailscale.com`) | `packages/mesh/src/drivers/tailscale.ts:12`; `packages/trpc/src/services/mesh.service.ts:620` | No (explicit driver) | Mesh control fails offline. | Keep as an opt-in driver. B1's in-cluster Headscale uses the same client image. |
...
4. **B2(b)** cluster zone on swarmy-dns (the `geo-edge-routing` skill owns it),
   plus a bundled mmdb.
5. **B1** in-cluster Headscale as the default mesh (the `mesh-networking` skill
   owns it). The installer's `netbird-cloud` choice moves under "external
   control plane".

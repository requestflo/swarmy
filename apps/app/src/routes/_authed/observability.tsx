...
function ServiceMetricsPanel({ enabled }: { enabled: boolean }): React.JSX.Element {
  const trpc = useTRPC();
  const [metric, setMetric] = React.useState<string>(METRIC_PRESETS[0].value);
        ...
        {!enabled ? (
          ...
          <div className="border-t">
            {rows.map((r) => (
              <div
                key={r.service_name}
                className="grid grid-cols-[1fr_auto] items-center gap-x-4 border-b px-6 py-3 last:border-b-0"
              >
                <div className="min-w-0">
                  <p className="mono-data truncate text-sm font-medium">{r.service_name}</p>
                  <div className="bg-muted/40 mt-1.5 h-1.5 w-full overflow-hidden rounded-full">
                    <div
                      className="bg-primary/70 h-full rounded-full"
                      style={{ width: `${Math.max(2, (r.avg_value / peak) * 100)}%` }}
                    />
                  </div>
                </div>

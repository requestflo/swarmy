...
export interface DeployPoll extends DeployProgress {
...
/**
 * Watch a fresh deploy by polling: the live inventory and the app's routes,
 * every 2 s while it is deploying and not at all once it is live. Step times
 * are what this tab saw — never invented — so a step already done on arrival
 * shows "done" without a time. The fallback under the streamed events.
 */
export function useDeployPoll(stack: string, result: BlueprintDeployResultView | null, startedAt: number | null, holdLive = false): DeployPoll {
  const trpc = useTRPC();
  ...
  // Keep polling while the streamed events say it isn't live yet.
  React.useEffect(() => setLive(progress.live && !holdLive), [progress.live, holdLive]);
  ...
  const doneAt = Object.fromEntries(Object.entries(times.current).filter(([, v]) => v >= 0)) as DeployPoll['doneAt'];

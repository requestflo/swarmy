...
export function DeployFlow({ stack, pinned }: { stack: string; pinned: boolean }): React.JSX.Element {
  ...
  const watch = useDeployProgress(stack, handoff?.result ?? null, startedAt);
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-6 pt-8 pb-28 lg:pb-16 xl:px-8">
        {liveAt !== null ? (
          <LiveView stack={stack} watch={watch} startedAt={startedAt} liveAt={liveAt} onLeave={leave} banner={banner} />
        ) : (
          <DeployingView stack={stack} watch={watch} startedAt={startedAt} banner={banner} />
        )}
      </div>
    </div>
  );

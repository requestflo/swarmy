...
/**
 * The apply/rollback confirm modal: a live restart preview ("These N services
 * will restart: …") before redeploying consumers onto `version`. Applying a
 * version older than the current one IS the rollback.
 */
export function ApplyConfigDialog({
  ...
  /** The version consumers move onto. */
  version: number;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            onClick={() => apply.mutate({ family: family.family, version })}
          ...
          >
            {apply.isPending
              ...
              : restarting.length > 0
                ...
                : `${rollback ? 'Roll back' : 'Apply'} v${version}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

    ...
    <section className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {del.isPending ? 'Deleting…' : confirming ? 'Click again to confirm' : 'Delete config'}
        </Button>
      </div>
      <p className="text-muted-foreground text-xs">
        {inUse
          ? `Deletion is blocked while ${family.usedByCount} service(s) still use it — detach them first.`
          : 'Deleting removes every version from Docker. There is no undo — pruned versions cannot be rolled back to.'}
      </p>
    </section>
  );
}

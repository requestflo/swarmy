...
/** Row tone: stale consumers need an apply nudge; unused is just quiet. */
export function familyTone(f: ConfigFamilyView): { tone: StatusTone; label: string } {
...
/** Flat family rows in one card: name, current version, consumers, updated. */
export function FamiliesTable({
  families,
  onOpen,
}: {
  families: ConfigFamilyView[];
  onOpen: (f: ConfigFamilyView) => void;
}): React.JSX.Element {
  return (
            ...
            >
              <span className="min-w-0">
                ...
                <span className="text-muted-foreground block truncate text-xs">
                  {f.versions.length} version{f.versions.length === 1 ? '' : 's'} · mounts at{' '}
                  {f.mountPath}
                </span>
              </span>
              ...
              <span className="mono-data text-muted-foreground hidden text-right text-xs lg:block">
                {relTime(f.lastUpdatedAt)}
              </span>
              <span className="mono-data hidden text-right text-sm lg:block">{f.usedByCount}</span>

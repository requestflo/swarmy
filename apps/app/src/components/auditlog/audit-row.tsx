...
import { Collapsible, CollapsibleContent, StatusBadge, cn } from '@swarmy/ui';
...
/** One timeline row: time · actor · humanized sentence · raw action chip. Expands inline. */
  ...
  return (
    <div className={cn(expanded && 'bg-accent/40 shadow-[inset_3px_0_0_var(--primary)]')}>
      <button
        type="button"
        onClick={onToggle}
        className="hover:bg-accent grid w-full grid-cols-[7rem_1fr_auto] items-center gap-x-4 gap-y-1 px-4 py-3 text-left transition-colors sm:grid-cols-[7rem_13rem_1fr_auto_1.5rem]"
      >
        <span
          className="mono-data text-muted-foreground text-xs"
        ...
        <ChevronRightIcon
          className={cn(
            'text-muted-foreground hidden size-4 shrink-0 transition-transform sm:block',
            expanded && 'rotate-90',
          )}
        />
      </button>

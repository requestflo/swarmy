...
import { ArrowUpRightIcon, LayersIcon, MoonIcon, ShieldIcon } from 'lucide-react';
 ...
 *
 * swarmy's own platform stack (`system: true`) renders visibly subdued — a
 * shield icon + "System" badge instead of the hover arrow, muted card fill —
 * so it reads as plumbing, not something you deployed.
 */
export function StackCard({ stat, onOpen }: StackCardProps): React.JSX.Element {
  const { label, ungrouped, system, tone, serviceCount, running, desired, linkCount, services } =
    stat;
  ...
  return (
    <button
      ...
      className={cn(
        ...
        system && 'bg-muted/40 shadow-none',
      )}
    >
      <div className="flex items-center gap-2.5">
        ...
        <span
          className={cn(
            ...
            (ungrouped || system) && 'text-muted-foreground',
          )}
        >
          {system ? 'swarmy system' : label}
        </span>
        {system ? (

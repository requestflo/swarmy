...
import { HistoryIcon, PencilIcon, PlayIcon, Trash2Icon } from 'lucide-react';
import type { ScheduledJobView } from '@swarmy/core';
import { Button, Switch, toast, Tooltip, TooltipContent, TooltipTrigger } from '@swarmy/ui';
...
/** One flat job row: what runs, when, how the last run went, what's next. */
  ...
  return (
    <div className="hover:bg-accent/40 grid grid-cols-2 items-center gap-x-4 gap-y-1 px-4 py-3 sm:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_8rem_9rem_auto]">
      ...
      <div className="flex flex-col gap-0.5">
        {job.lastRunStatus ? <JobRunChip status={job.lastRunStatus} /> : (
      ...
      <div className="col-span-2 flex items-center justify-end gap-1 sm:col-span-1">
        ...
        <IconAction label="Run history" onClick={() => onHistory(job)}>
          <HistoryIcon className="size-4" />
        </IconAction>
        <IconAction label="Edit" onClick={() => onEdit(job)}>
          <PencilIcon className="size-4" />
        </IconAction>
        <IconAction label="Delete" onClick={() => onDelete(job)}>

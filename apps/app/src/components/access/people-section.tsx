...
export function PeopleSection({ canInvite }: { canInvite: boolean }): React.JSX.Element {
  ...
  const robots = (keys.data ?? []).filter((k) => !k.revokedAt);
  return (
    <>
      <Section id="people" title="People" count={rows.length} flush>
        <RowList label="People">
          ...
          {robots.map((k) => (
            <CalmRow key={k.id} tone="idle" name={k.name} sub={`API key · ${k.prefix}…`} say={`Scoped to ${k.scopes.join(', ')}`} tech={`last used ${relTime(k.lastUsedAt)}`} word="robot" wordTone="idle" to="/settings/api-keys" />
          ))}
        </RowList>

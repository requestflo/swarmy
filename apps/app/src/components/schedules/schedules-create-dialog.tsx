...
interface SchedulesCreateDialogProps {
  targets: TargetOption[];
}

/** Coral CTA → create a recurring volume backup. Mirrors the ingress/store UX. */
export function SchedulesCreateDialog({ targets }: SchedulesCreateDialogProps): React.JSX.Element {
  const trpc = useTRPC();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          className="shadow-[0_8px_24px_-8px_var(--primary)] transition-transform hover:scale-[1.03]"
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            ...
            onClick={() => create.mutate({ targetId, volume, every: Number(every), unit })}
          >
            Create schedule
          </Button>
        </DialogFooter>

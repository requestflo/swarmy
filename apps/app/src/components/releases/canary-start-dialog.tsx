...
/**
 * Start-canary dialog: pick a service in the stack, the candidate image, the
 * traffic share, the watch window and the error-rate ceiling. On submit the
 * controller deploys `<svc>--canary` and shifts that share of the ingress
 * traffic to it — the deploy-canary worker promotes or rolls back from there.
 */
export function CanaryStartDialog({ stackName }: { stackName: string }): React.JSX.Element {
  const trpc = useTRPC();
  ...
  const services = useQuery({
    ...trpc.services.list.queryOptions({ stackId: stackName }),
    enabled: open,
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Start a canary on {stackName}</DialogTitle>
          <DialogDescription>

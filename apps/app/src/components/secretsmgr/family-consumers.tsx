...
/** Who reads this secret: consumer rows with links + one-click detach. */
export function FamilyConsumers({ family }: { family: SecretFamilyView }): React.JSX.Element {
  const trpc = useTRPC();
  const qc = useQueryClient();

  const detach = useMutation(
    trpc.secrets.detach.mutationOptions({
      onSuccess: (r) => {
    ...
    <section className="space-y-2">
      ...
      {family.consumers.length === 0 ? (
        <p className="text-muted-foreground text-xs">
          Nothing reads this secret yet — attach it to a service below.
        </p>
      ) : (

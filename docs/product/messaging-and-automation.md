# Messaging & automation — "queues, jobs and webhooks, native to the swarm"
...
1. They open a stack, click **Messaging**, and see three sections stacked for the
   one app — **Queues**, **Webhooks**, **Scheduled jobs** — each
...
4. They give Stripe a **public URL** to paste — `POST /hooks/i/<org>/<slug>`.
...
5. They point a **Scheduled job** at `cron 0 3 * * *` running a backup image,
   and subscribe their own URL to swarmy's **outbound webhooks** so their pager
   hears about a failed deploy the moment it happens.
...
- **Jobs are the DB's to keep, because they are history.** A `ScheduledJob` is
  the user's *input artifact* (like a stack's compose source); a `JobRun` is
  *queryable run history*. That is exactly what swarmy's DB is for. Nothing about
  live swarm state is mirrored — targets resolve from live inventory at run time.
- **Every side effect is one signature.** Inbound HMAC/GitHub/Stripe verify
  and outbound event delivery all use the same
...
- **Queue contents live in the cache cluster, never swarmy.** Waiting/active/
  ...
  wire).
- **Scheduled jobs and their runs are swarmy's DB** — `ScheduledJob`/
  `JobRun`. These are identity +
...
- **What a job/step actually runs is resolved live.** An image job dispatches
  `container.runOnce`; a service-exec job resolves a running task container
...
| No online node to run a job | The attempt fails with "no online node"; a scheduled job records the failed `JobRun` and, with `alertOnFailure`, raises an alert. |
| Controller restarts mid-run | State lives in `control.db` (cursor, stateJson) + the cache; the next tick picks up PENDING deliveries and due jobs. No work is lost, none double-commits (guarded writes). |
...
- **A workflow engine (multi-step runs with approvals).** Built once, removed
  before launch (2026-09): it could only be started by hand and nothing else
  used it. A scheduled job plus a queue covers the automation cases we promise.
- **Forwarding inbound webhooks anywhere the payload names.** Forward targets are
  ...
  the request body.
- **Returning any secret to the client.** Verify secrets and outbound signing
  secrets are write-only: stored `…SecretEnc`, surfaced only
...
- **UI** — the stack workspace **Messaging** tab
  ...
  `stack-queues-section`, `stack-webhooks-section` and `stack-jobs-section` under
  `apps/app/src/components/{queues,webhookgw,jobs}/`.

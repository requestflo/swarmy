...
Postgres password, the swarm join tokens and the admin password. The installer
...
does not remember most flags: `--standard`, `--admin-email`, `--port`,
...
- A **standard** (Postgres) install re-run without `--standard` redeploys the
  **lite** stack. The controller then comes up on an empty embedded database.
  Your Postgres volume is untouched, and re-running with `--standard` brings it
  back.
...
curl -fsSL https://raw.githubusercontent.com/requestflo/swarmy/main/scripts/install-swarmy.sh \
  | sudo bash -s -- --non-interactive --admin-email you@example.com   # + --standard etc., as installed
```
...
- It runs `docker stack deploy` again. That re-resolves the image tag against
  ...
  controller applies any pending database migrations itself, on both lite and
  standard. The bootstrap seed is idempotent, so no new org, owner or token
  appears.
...
| `swarmy_swarmy-data` volume (lite) / `swarmy_swarmy-pgdata` volume (standard) | The controller's database |

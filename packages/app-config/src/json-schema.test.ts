...
import { EmailObjectSchema } from './email';
...
describe('JSON Schema ↔ Zod drift guard', () => {
  ...
  it('top level, service, job and previews agree', () => {
    ...
    expect(jkeys(S.properties.ai)).toEqual(zkeys(AiSchema));
    expect(jkeys(S.properties.email.anyOf[1])).toEqual(zkeys(EmailObjectSchema));

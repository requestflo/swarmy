/**
 * Geo steering — pure, IO-free record selection (epic #12, Part A — PHASE 2).
 *
 ...
 *
 * The controller renders these decisions into the CoreDNS zone (so failover =
 * dropping unhealthy regions from the answer set). Because resolution is pure it
 * is exhaustively unit-tested and reused by both the zone renderer and the
 * health-aware reconcile worker.
 *
 * No external GeoIP DB is bundled here: region coordinates are a small static
 * table (the `swarmy.region` label values map to lat/lon). A MaxMind lookup, if
 * configured, only converts a *client IP/subnet* → a `ClientLocation`; the
 * ranking math below is provider-agnostic. See the INTEGRATION section for the

...
describe('storage-offsite check', () => {
  it('flags schedules that only ever write inside the cluster', () => {
    const p = byCheck(runChecks(healthySnapshot({ offsite: { schedules: 3, offsiteSchedules: 0 } })), 'storage-offsite');
    expect(p).toHaveLength(1);
    expect(p[0]!.title).toBe('Backups have no off-site copy');
    expect(p[0]!.severity).toBe('warn');
    expect(p[0]!.fixPath).toBe('/backups');
  });

  it('is quiet once any schedule copies off-site, or when nothing is scheduled', () => {
    expect(byCheck(runChecks(healthySnapshot({ offsite: { schedules: 3, offsiteSchedules: 1 } })), 'storage-offsite')).toHaveLength(0);
    expect(byCheck(runChecks(healthySnapshot({ offsite: { schedules: 0, offsiteSchedules: 0 } })), 'storage-offsite')).toHaveLength(0);

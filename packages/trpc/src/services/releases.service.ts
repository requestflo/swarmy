 ...
 * payload, swapping in `image`. Thin wrapper over {@link specFromInspect} (the
 * shared live-spec codec every one-aspect patch rides — see service-patch.ts).
 * Network NAMES come from the live inventory (raw inspect only carries network
 * ids). Returns null when the inspect payload is unusable.
 */
export function promoteSpecFrom(inspect: unknown, image: string, networks: string[]): ServiceSpec | null {
  const spec = specFromInspect(inspect, networks);
